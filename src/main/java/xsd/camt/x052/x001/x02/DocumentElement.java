...
public class DocumentElement extends biz.c24.io.api.data.Element {
    ...
    private DocumentElement(boolean init) {
        super("Document", 1, 1, iso.std.iso.x20022.tech.xsd.camt.x052.x001.x02.DocumentCls.class, iso.std.iso.x20022.tech.xsd.camt.x052.x001.x02.Camt05200102DataModel.getInstance());
    ...
    public static biz.c24.io.api.data.Element getInstance() {
        if (!initialized)
        {
            synchronized (biz.c24.io.api.data.ComplexDataType.class)
            {
                if (instance == null)
                {
                    instance = new iso.std.iso.x20022.tech.xsd.camt.x052.x001.x02.DocumentElement(false);
                    instance.init();
                    initialized = true;
                }
            }
